...
<template>
  <div class="app-container">
    ...
    <!-- 工具条 -->
    <div>
      <el-button type="danger" size="mini" @click="addBanner()">添加</el-button>
    ...
    <el-table
      ...
      <el-table-column label="banner" width="420" align="center">
        <template slot-scope="scope">
          <div class="info">
            <div class="pic">
...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      ...
      bannerApi.getPageList(this.page, this.limit, this.searchObj).then(
        response => {
    ...
    removeDataById(id) {
      ...
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        // 点击确定，远程调用ajax
        return bannerApi.removeById(id)
      }).then((response) => {
    ...
    removeRows() {
      ...
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        // 调用api
        return bannerApi.removeRows(idList)
      }).then((response) => {
        this.fetchData(this.page)
        if (response.success) {
          this.$message({
